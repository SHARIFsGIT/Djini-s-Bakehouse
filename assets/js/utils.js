...
class DjinisUtils {
  ...
  /**
   * Format shipping time estimate
   * @param {number} minDays - Minimum days
   * @param {number} maxDays - Maximum days
   * @returns {string} Formatted shipping estimate
   */
  static formatShippingEstimate(minDays, maxDays) {
    if (minDays === maxDays) {
   ...
   * @param {number} shippingDays - Number of shipping days
   * @param {boolean} businessDaysOnly - Count business days only
   * @returns {Date} Estimated delivery date
   */
  static getEstimatedDeliveryDate(shippingDays, businessDaysOnly = true) {
    const date = new Date();
    let daysAdded = 0;
    ...
    while (daysAdded < shippingDays) {
      ...
      // Skip weekends if counting business days only
      if (businessDaysOnly) {
   ...
   */
  static setCookie(name, value, days = 30, path = "/") {
    const date = new Date();
